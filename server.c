...
#endif
{
    ...
    /* Step 2: Generate and send server nonce */
    ...
    /* Step 3: Connect to local target service (e.g. sshd) */
    ...
    lk_chacha20_ctx ctx_dec; /* Decrypt incoming client-to-server traffic */
    lk_chacha20_ctx ctx_enc; /* Encrypt outgoing server-to-client traffic */
    ...
    while (1) {
        ...
        /* Forward from tunnel (encrypted) -> target (plain) */
        if (FD_ISSET(sock_tunnel, &read_fds)) {
            ...
            if (write_exact(sock_target, buf_in, (size_t)n) != 0) {
                break;
            }
        }

        /* Forward from target (plain) -> tunnel (encrypted) */
        if (FD_ISSET(sock_target, &read_fds)) {
            ...
            if (write_exact(sock_tunnel, buf_out, (size_t)n) != 0) {
                break;
            }
        }
    }
...
static void print_usage(const char *prog) {
    ...
    fprintf(stderr, "  -l, --listen <host:port>   Listen host:port (default: 0.0.0.0:8443)\n");
    fprintf(stderr, "  -t, --target <host:port>   Target host:port (default: 127.0.0.1:22)\n");
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        if ((strcmp(argv[i], "-l") == 0 || strcmp(argv[i], "--listen") == 0) && i + 1 < argc) {
            ...
            if (parse_host_port(argv[++i], target_host, sizeof(target_host), &target_port) != 0) {
    ...
    while (1) {
        ...
        if (spawn_thread(handle_client, conn) != 0) {
            CLOSE_SOCK(client_sock);
            free(conn);
        }
    }
